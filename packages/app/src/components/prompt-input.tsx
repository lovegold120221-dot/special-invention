...
import { createAudioRecorder, type RecorderState } from "./prompt-input/audio-recorder"
import { LANGUAGES, type LanguageCode } from "./prompt-input/languages"
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  // Persisted STT language preference (default: English)
  const [sttPrefs, setSttPrefs, _, sttPrefsReady] = persisted(
    Persist.global("sttLanguage", ["sttLanguage.v1"]),
    createStore({ sttLanguage: "en" as LanguageCode }),
  )

  const getRecorder = () => {
    if (!recorder) {
      recorder = createAudioRecorder()
      recorder.onAudioLevel = (level) => setAudioLevel(level)
    }
    return recorder
  }
  ...
  const toggleRecording = async () => {
    ...
    if (current === "recording") {
      ...
      try {
        ...
        if (!platform.sttTranscribe) {
          showToast({
            variant: "error",
            title: language.t("common.requestFailed"),
            description: "Voice transcription is not available on this platform",
          })
          setRecorderState("idle")
          return
        }
        const text = await platform.sttTranscribe(blob, sttPrefs.sttLanguage)
        if (text) {
  ...
  // Compact language dropdown for STT transcription
  const LanguageDropdown = () => (
    <select
      value={sttPrefs.sttLanguage}
      onChange={(e) => setSttPrefs("sttLanguage", e.target.value as LanguageCode)}
      onClick={(e) => e.stopPropagation()}
      class="h-5 rounded border-none bg-transparent px-0.5 text-[11px] text-v2-icon-icon-muted outline-none
        hover:text-text-base cursor-pointer appearance-none"
      style={{ direction: "rtl" /* show arrow on left side to save space */ }}
  ...
  // Audio frequency bars visualizer — appears during recording
  const FreqBars = () => {
    const level = audioLevel()
    // 5 bars with staggered heights for a natural equalizer look
    const barHeights = [
      Math.max(3, 3 + level * 16 * 0.5),
      Math.max(3, 3 + level * 16 * 0.8),
      Math.max(3, 3 + level * 16),
      Math.max(3, 3 + level * 16 * 0.7),
      Math.max(3, 3 + level * 16 * 0.4),
    ]
    return (
      <Show when={recorderState() === "recording"}>
        <div class="flex items-end gap-[2px] h-4 px-1" aria-hidden="true">
          <For each={barHeights}>
            {(h) => (
              <div
                class="w-[3px] rounded-[1px] bg-v2-icon-icon-muted"
                style={{ height: `${h}px`, transition: "height 0.06s ease" }}
              />
            )}
          </For>
        </div>
  ...
  // Listen for the voice-toggle custom event dispatched by the command system
  if (typeof document !== "undefined") {
    const handler = () => { if (store.mode === "normal") toggleRecording() }
    document.addEventListener("voice-toggle", handler)
    onCleanup(() => document.removeEventListener("voice-toggle", handler))
  }
              ...
              <div class="flex h-11 items-center px-2">
                <div class="flex min-w-0 flex-1 items-center gap-0">
                  ...
                  <TooltipKeybind
                    ...
                    title={
                      recorderState() === "idle"
                        ? language.t("prompt.action.mic")
                        : recorderState() === "recording"
                          ? language.t("prompt.action.mic.stop")
                          : language.t("prompt.action.mic.transcribing")
                    }
                    keybind={command.keybind("voice.toggle")}
                  >
                    <IconButton
                      data-action="prompt-mic"
                      type="button"
                      icon={recorderState() === "recording" ? "stop" : "mic"}
                      variant="ghost"
                      class="size-7 rounded-md p-[6px] text-v2-icon-icon-muted"
                      style={buttons()}
                      onClick={toggleRecording}
                      disabled={store.mode !== "normal" || recorderState() === "transcribing"}
                      tabIndex={store.mode === "normal" ? undefined : -1}
                      aria-label={
                        recorderState() === "idle"
                          ? language.t("prompt.action.mic")
