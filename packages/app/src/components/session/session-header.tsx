...
import { fetchProfiles, ENGINES, type VoiceboxProfile } from "@/services/voicebox"
import { speakAndPlay } from "@/services/voicebox"
...
export function SessionHeader() {
  ...
  const [ttsPrefs, setTtsPrefs] = persisted(
    Persist.global("tts", ["tts.v1"]),
    createStore({ enabled: false, profileId: "", engine: "luxtts" }),
  )
  const [profiles, setProfiles] = createSignal<VoiceboxProfile[]>([])

  onMount(() => {
    fetchProfiles().then(setProfiles)
  })

  // Auto-read hook: listen for session.idle events and read the last assistant message
  if (typeof document !== "undefined") {
    const autoReadHandler = (e: Event) => {
      if (!ttsPrefs.enabled) return
      const detail = (e as CustomEvent).detail as { text?: string } | undefined
      if (!detail?.text) return
      speakAndPlay(detail.text, ttsPrefs.profileId || undefined, ttsPrefs.engine).catch(() => {})
    }
    document.addEventListener("opencode-tts-read", autoReadHandler)
    onCleanup(() => document.removeEventListener("opencode-tts-read", autoReadHandler))
  }
