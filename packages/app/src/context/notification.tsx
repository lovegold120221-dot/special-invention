  ...
  init: () => {
    ...
    const handleSessionIdle = (directory: string, event: { properties: { sessionID?: string } }, time: number) => {
      ...
      void lookup(directory, sessionID).then((session) => {
        ...
        // Dispatch TTS event for auto-read feature
        if (sessionID && typeof document !== "undefined") {
          // Look up last assistant message text from the sync store
          const [syncStore] = serverSync.child(directory, { bootstrap: false })
          const msgs = syncStore.message[sessionID]
          let text = ""
          if (msgs) {
            ...
            if (lastAssistant) {
              const parts = syncStore.part[lastAssistant.id]
              if (parts) {
          ...
          if (text.trim()) {
            document.dispatchEvent(
              new CustomEvent("opencode-tts-read", { detail: { text: text.trim() } }),
            )
          }
        }
