...
import { getSttState, startSttRecording, stopSttAndTranscribe, abortSttRecording } from "../../audio"
...
export function Prompt(props: PromptProps) {
  ...
  // Poll STT state during recording
  createEffect(() => {
    const current = store.sttState
    if (current === "idle" || current === "transcribing") return
    const interval = setInterval(() => {
      const state = getSttState()
      if (state !== store.sttState) {
        setStore("sttState", state)
      }
  ...
  const toggleVoice = async () => {
    ...
    if (current === "recording") {
      setStore("sttState", "transcribing")
      try {
        const text = await stopSttAndTranscribe()
        if (text) {
          const insert = text + " "
          if (input && !input.isDestroyed) {
            const currentOffset = input.cursorOffset
            input.insertText(insert)
            input.cursorOffset = currentOffset + insert.length
            // Update store
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        desc: "Start or stop microphone recording for voice transcription",
  ...
  useBindings(() => {
    return {
      ...
      bindings: [{
        key: "alt+m",
        desc: "Toggle voice input",
        group: "Prompt",
        cmd: () => void toggleVoice(),
      }],
    }
  })
        ...
        <box width="100%" flexDirection="row" justifyContent="space-between">
          ...
          <Show when={status().type !== "retry"}>
            <box gap={2} flexDirection="row">
              ...
              <Show when={store.sttState !== "idle"}>
                <text fg={store.sttState === "recording" ? theme.error : theme.warning}>
                  {store.sttState === "recording" ? "[● Mic]" : "[⋯ Transcribing]"}
                </text>
              </Show>
              <Switch>
